...
class Database:
    ...
    def __init__(self):
        ...
        self.cursor.execute(
            f"SELECT name FROM sqlite_master WHERE type='table' AND name='{Database.table_name}'"
        )
        ...
        if result:
            ...
            self.cursor.execute(
                f"CREATE TABLE {Database.table_name} (id INTEGER PRIMARY KEY, state bool);"
            )  # Uncomment this line and fill out thing
